...
from model import PropagateCount # 一定时间、话题、信息类型对应的{domain:count}
...
def quicknessCronTOpic(topic, xapian_search_weibo, start_ts, over_ts, during=Fifteenminutes):
     if topic and topic != '':
        ...
        for i in range(interval, 0, -1): # 一个15分钟，计算出一个quickness
            ...
            for k, v in mtype_kv,iteritem():
                quert_dict['message_type'] = v
                ...
                for r in domain_ts():
                    ...
                    save_qc_results(topic, topnum, allnum, during, end_ts, v, uiddomain) # 存入Quickness表
