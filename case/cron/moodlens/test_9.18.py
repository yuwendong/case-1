...
from dynamic_xapian_weibo import getXapianWeiboByDate, getXapianWeiboByTopic
...
count, results = s.search(query={'topics': [u'东盟', u'博览会'], '$or':[{'message_type':1 , 'message_type':3}]}, fields=['_id'])#fields=fields_list)    
f = open('test_mid.txt', 'wb')
for result in results():
    ...
    city = geo2city(geo)
    save_line = str(user)+'  '+ result['text'].encode('utf-8') + '   ' + otherStyleTime + ' ' + city
    '''
    save_line = str(result['_id'])
    f.write(save_line+'\n')
print 'all_count:', k
'''
stopic=u'中国'
