...
function refreshDrawWeibos(select_name, weibos_obj){
    ...
    $("#weibos_div").css("height", $("#weibo_list").css("height"));
...
TrendsLine.prototype.pullDrawTrend = function(){
    ...
    $("[name='abs_rel_switch']").on('switchChange.bootstrapSwitch', function(event, state) {
        var chart = $(that.trend_chart).highcharts();
        ...
        if (state == false){
            ...
            for (var i in chart.series){
                ...
                if(i == 0 || i == 1 || i == 2){
                    ...
                    series.update({
                        data: that.trend_count_obj['incre'][name]
                    });
                }
                else if (i == 3 || i == 4 || i == 5){
                    series.update({
...
function pull_emotion_count(that, query, emotion_type, total_days, times, begin_ts, during, count_series, count_peak_series, incre_peak_series){
    var names = that.names;

    if(times > total_days){
        get_peaks(that, count_peak_series, that.trend_count_obj['count'], that.trend_count_obj['ts'], during);
        //get_peaks(that, incre_peak_series, that.trend_count_obj['incre'], that.trend_count_obj['ts'], during);
